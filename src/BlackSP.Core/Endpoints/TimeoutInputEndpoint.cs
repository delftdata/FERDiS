...
{
    public class TimeoutInputEndpoint<TMessage> : IInputEndpoint, IDisposable
        where TMessage : IMessage
    {
        ...
        /// <returns></returns>
        public delegate TimeoutInputEndpoint<TMessage> Factory(string endpointName);
        ...
        public async Task Ingress(Stream s, string remoteEndpointName, int remoteShardId, CancellationToken t)
        {
            ...
            try
            {
                ...
                _logger.Debug($"Input endpoint {_endpointConfig.LocalEndpointName} starting read & deserialize threads. Reading from \"{_endpointConfig.RemoteVertexName} {remoteEndpointName}\" on instance \"{_endpointConfig.GetRemoteInstanceName(remoteShardId)}\"");
